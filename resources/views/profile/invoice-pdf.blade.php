...
<body>
    <div class="invoice-container">
        ...
        <div style="text-align: center;">
                @if($purchase->status === 'success')
                <a href="https://wa.me/{{ config('app.payment.whatsapp_number') }}?text={{ urlencode('Hello MersifLab, I have a question about the payment for invoice ' . $purchase->purchase_code . ' which was successful for Rp' . number_format($purchase->amount, 0, ',', '.') . '. Thank you!') }}" 
                   class="whatsapp-button" target="_blank">
                    Contact Admin
                </a>
            @else
                <a href="https://wa.me/{{ config('app.payment.whatsapp_number') }}?text={{ urlencode('Hello MersifLab, I want to confirm payment for invoice ' . $purchase->purchase_code . ' for Rp' . number_format($purchase->amount, 0, ',', '.')) }}" 
                   class="whatsapp-button" target="_blank">
